...
class RnaEdit(QtCore.QThread):
    ...
    def __init__(self, fastqFiles, params, textField):
	    ...
	    #En plus
            ...
            #MORE
	    self.sampleName=fastqFiles[0].split("/")[-1].split(".")[0]
	    
	    path=("/").join(fastqFiles[0].split("/")[:-1])
	    self.outdir=path+"/rnaEditor/Res_"+self.sampleName+"/"
	    ######
	...
	path_RNAEditor=os.environ.get("RNAditing_PATH_RNAEditor")
        cmd=["python",path_RNAEditor+"createDiagrams.py","-o", self.params.output]
